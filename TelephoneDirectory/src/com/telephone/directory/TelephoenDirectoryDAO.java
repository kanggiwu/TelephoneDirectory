...
public class TelephoenDirectoryDAO {
	...
	public void db_search() {
		...
		try {
			...
			StringBuilder sql = getQuery(combo_index);
			// 연결통로확보 하기
	...
	public void getTelvo(String click_name) {
		...
		StringBuilder sql = new StringBuilder();
		sql.append(
				"SELECT store_name, tel_num, t_name, food_style, address, main_dish, seq FROM telephonebook WHERE seq=?");
		// 값 가져오기////////////////////////////////////////
		try {
	...
	public void delete() {
		...
		dbMgr = DBConnectionMgr.getInstance();
		StringBuilder sql = new StringBuilder();
		sql.append("DELETE FROM telephonebook");
	...
	public StringBuilder getQuery(int combo_index) {
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT store_name, address, tel_num, food_style,seq FROM telephonebook WHERE ");
		switch (combo_index) {
	...
	public int rowNum_findmax() {
		...
		StringBuilder sql = new StringBuilder();
		ResultSet rs = null;
		sql.append("SELECT MAX(seq) FROM telephonebook");
		try {
			con = dbMgr.getConnection();
			pstmt = con.prepareStatement(sql.toString());
			rs = pstmt.executeQuery();
	...
	public void db_insert() {
		this.telVO = new TelVO();
		telVO.setSeq(rowNum_findmax());//seq에 가장 큰 값을 넣어준다. pk
		DBConnectionMgr dbMgr = DBConnectionMgr.getInstance();
		Connection con = dbMgr.getConnection();
		PreparedStatement pstmt = null;
		StringBuilder sql = new StringBuilder();
		...
		telVO1.setAddress(t_view.t_dialog.jtf_address.getText());
		telVO1.setMain_dish(t_view.t_dialog.jtf_mainDish.getText());
		sql.append("INSERT INTO telephonebook VALUES (?,?,?,?,?,?,?)");
		if (telVO1.getStore_name().isEmpty() || telVO1.getTel_num().isEmpty()) {
			JOptionPane.showMessageDialog(t_view.t_dialog, "음식점 이름 또는 전화번호를 입력하시오.");
